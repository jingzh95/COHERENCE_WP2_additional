...
var2 <- apply(tempall, 2,var)
temp34<-as.data.frame(var2[which(var2>0.001)]) 
temp34$var3 <- rownames(temp34)
tempall<- tempall[,temp34$var3]
...
temp35$var3 <- rownames(temp35)

temp35 <- temp35 %>% 
  filter(!str_detect(var3, var30))
...
write.xlsx(temp36[,c("var3", "var35b")],paste0(result_path,"Bar_ensemble_score_",
                                               file_name,
                                               ".xlsx"))



tempall2=cbind(tempall1[,c(temp36$var3)],var4=tempall1$var4)
...
plotPeaks = function(df,i){
  p = findpeaks(temp39,minpeakdistance=i,minpeakheight=0)
  plot(1:length(temp39),main=paste("minpeakdistance=",i))
  abline(v=p[,2])
}
...
temp35 <- temp35 %>% 
  filter(!str_detect(var3, var30))  #, var30
...
temp41 <- round(temp41)
tempnow <- as.data.frame(cbind(temp41,temp36$var3, temp36$var8))
...
temp37<-tempall3
for (i in 1:(ncol(tempall3)-1)){
  tempall3[,i]<-tempall3[,i]*temp41[i]
}
